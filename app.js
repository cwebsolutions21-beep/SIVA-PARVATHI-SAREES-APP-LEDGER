...
function loadFast2SMSWhatsAppKeys() {
    let key = localStorage.getItem('fast2sms_waba_key') || '';
    if (!key) {
        key = 'B9tOEby7xYWzojcawmq5piGKD2RdC1IuZ6NrTVMhJH843gLUSXFQqRgKDbz41xcM7PrUsdWZ5e9AypBm';
        localStorage.setItem('fast2sms_waba_key', key);
    }
    let phoneId = localStorage.getItem('fast2sms_waba_phone_id') || '';
    if (!phoneId) {
        phoneId = '805287855998158';
        localStorage.setItem('fast2sms_waba_phone_id', phoneId);
    }
    let templateId = localStorage.getItem('fast2sms_waba_template_id') || '';
    if (!templateId) {
        templateId = '3948714455438137';
        localStorage.setItem('fast2sms_waba_template_id', templateId);
    }

    const keyInput = document.getElementById('fast2smsWabaKey');
    const phoneIdInput = document.getElementById('fast2smsWabaPhoneId');
    const templateIdInput = document.getElementById('fast2smsWabaTemplateId');
...
function saveFast2SMSWhatsAppKeys() {
    const keyInput = document.getElementById('fast2smsWabaKey');
    const phoneIdInput = document.getElementById('fast2smsWabaPhoneId');
    const templateIdInput = document.getElementById('fast2smsWabaTemplateId');

    if (keyInput) localStorage.setItem('fast2sms_waba_key', keyInput.value.trim());
    if (phoneIdInput) localStorage.setItem('fast2sms_waba_phone_id', phoneIdInput.value.trim());
    if (templateIdInput) localStorage.setItem('fast2sms_waba_template_id', templateIdInput.value.trim());
}

function toggleWabaKeyVisibility() {
    const input = document.getElementById('fast2smsWabaKey');
...
function remindBorrowerSMS(i) {
    const b = state.borrowers[i];
    const bal = Number(b.amount) - (b.payments || []).reduce((a, c) => a + Number(c.amount), 0);
    const msg = `నమస్కారం ${b.name}, శివ పార్వతి సారీస్ షాపు నుండి గుర్తు చేయునది. మీ బాకీ మొత్తం ₹${bal} ఇంకా చెల్లించాల్సి ఉంది. ధన్యవాదాలు!`;
    window.open(`sms:${b.phone}?body=${encodeURIComponent(msg)}`, '_blank');
}
...
function sendNextSingleTabReminderDirectly() {
    ...
    if (activeReminderMode === 'whatsapp') {
        ...
        if (apiKey) {
            // Automated background custom SMS via Fast2SMS
            fetch("https://www.fast2sms.com/dev/bulkV2", {
                ...
                body: JSON.stringify({
                    ...
                    "language": "english",
                    "flash": 0,
                    "numbers": b.phone.trim()
                })
            }).then(response => response.json())
              .then(result => {
                  if (result.return) {
                      console.log("Custom SMS sent successfully to", b.name);
                  } else {
                      console.error("Fast2SMS error for", b.name, ":", result.message);
                  }
              }).catch(err => console.error("Fast2SMS fetch error:", err));
        } else {
            // Open native SMS link
            window.open(`sms:${b.phone}?body=${encodeURIComponent(msg)}`, '_blank');
        }
    }
...
async function sendGroupSMS() {
    ...
    if (apiKey) {
        if (!confirm(`Are you sure you want to send automated background SMS to all ${list.length} borrowers using your Fast2SMS wallet?`)) {
            return;
        }
        ...
        if (statusCard) statusCard.style.display = 'block';
        if (progressText) {
            progressText.innerHTML = `<span style="color:var(--accent); font-weight:bold;"><i class="fas fa-spinner fa-spin"></i> Dispatched bulk SMS to Fast2SMS gateway...</span>`;
        }
        ...
        try {
            ...
            if (response.ok && result.return) {
                alert(`Successfully sent automated bulk SMS to all ${list.length} borrowers via Fast2SMS!`);
            } else {
                alert(`Fast2SMS Error: ${result.message || "Failed to send. Please check your wallet balance."}`);
            }
        } catch (err) {
            console.error("Fast2SMS API Call Error:", err);
            alert("Error connecting to Fast2SMS server. Please check your internet connection.");
        } finally {
            if (statusCard) statusCard.style.display = 'none';
        }
    } else {
        ...
        const phoneList = phones.join(separator);
        const bodyParam = isIOS ? '&body=' : '?body=';
        const smsLink = `sms:${phoneList}${bodyParam}${encodeURIComponent(msg)}`;
        window.open(smsLink, '_blank');
    }
}

async function sendWhatsAppAPIBulk() {
    const key = (localStorage.getItem('fast2sms_waba_key') || '').trim();
    const phoneId = (localStorage.getItem('fast2sms_waba_phone_id') || '').trim();
    const templateId = (localStorage.getItem('fast2sms_waba_template_id') || '').trim();

    if (!key || !phoneId || !templateId) {
        alert("Please enter your Fast2SMS WhatsApp API Key, Phone Number ID, and Template ID on the Dashboard settings card first!");
        return;
    }
    ...
    for (let i = 0; i < bulkQueue.length; i++) {
        const b = bulkQueue[i];
        if (progressText) {
            progressText.innerHTML = `<span style="color:#00e676; font-weight:bold;"><i class="fas fa-paper-plane fa-spin"></i> Sending WhatsApp to ${b.name} (${i + 1}/${bulkQueue.length})...</span>`;
        }

        const formattedPhone = formatWhatsAppPhone(b.phone); // e.g. "919381515692"
        const variables = `${b.name}|${b.balance}`; // template variables e.g. "Name|Balance"

        try {
            // Call the Fast2SMS WhatsApp Business API GET endpoint
            const response = await fetch(`https://www.fast2sms.com/dev/whatsapp?authorization=${encodeURIComponent(key)}&message_id=${encodeURIComponent(templateId)}&phone_number_id=${encodeURIComponent(phoneId)}&numbers=${encodeURIComponent(formattedPhone)}&variables_values=${encodeURIComponent(variables)}`);
            ...
            if (response.ok && result.return) {
                ...
                lastErrorMsg = result.message || "Template is still PENDING or WABA Account is BLOCKED by Facebook.";
                console.error("Failed to send message to", b.name, lastErrorMsg);
            }
        } catch (err) {
            lastErrorMsg = "Network connection error.";
            console.error("Error calling Fast2SMS WhatsApp API:", err);
        }
    ...
    if (successCount === 0 && lastErrorMsg) {
        alert(`Fast2SMS WhatsApp API Error:\n\n"${lastErrorMsg}"\n\n(Please check if your template status is still PENDING in Fast2SMS, or if Facebook is blocking your number due to payment/timezone setup).`);
    } else {
