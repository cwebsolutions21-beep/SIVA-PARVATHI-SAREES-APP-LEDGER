...
const firebaseConfig = {
    apiKey: "AIzaSyCXk0ExK9BuPi2CB4beFyQDBpiDZ5GvcQA",
    authDomain: "sarees-12373.firebaseapp.com",
    databaseURL: "https://sarees-12373-default-rtdb.firebaseio.com",
    projectId: "sarees-12373",
    storageBucket: "sarees-12373.firebasestorage.app",
    messagingSenderId: "834552133167",
    appId: "1:834552133167:web:d8c77ab9ce4cb19e13f8da",
    measurementId: "G-0N1FXPMYM"
};
...
let state = { borrowers: [], sales: [], suppliers: [], password: SHOP_PASS };

// Force local storage backup to prevent refresh data loss
const localBackup = localStorage.getItem('sp_state_backup');
if (localBackup) { try { state = JSON.parse(localBackup); } catch(e){} }

document.addEventListener('DOMContentLoaded', () => {
    checkAuth();
    setupEventListeners();
    function updateDate() { if(document.getElementById('currentDate')) document.getElementById('currentDate').innerText = formatDate(new Date()); }
...
async function handleLogin() {
    const id = document.getElementById('login-shop-id').value.trim();
    const pass = document.getElementById('login-password').value.trim();
    
    if (!id || !pass) { alert("Please enter ID and Password"); return; }

    const status = document.getElementById('sync-status');
    if(status) status.innerHTML = '<i class="fas fa-sync fa-spin"></i> Checking...';

    try {
        // Check Firebase for this shop
        const snapshot = await db.ref(`shops/${id}/password`).once('value');
        const existingPass = snapshot.val();

        if (!existingPass) {
            // Create new account
            if (confirm("Create new shop account with this ID and Password?")) {
                currentUser = { id, pass };
                localStorage.setItem('sp_user', JSON.stringify(currentUser));
                checkAuth();
            }
        } else if (existingPass === pass) {
            // Success login
            currentUser = { id, pass };
            localStorage.setItem('sp_user', JSON.stringify(currentUser));
            checkAuth();
        } else {
            alert("Incorrect password for this Shop ID!");
            if(status) status.innerHTML = '<i class="fas fa-times-circle"></i> Auth Failed';
        }
    } catch (e) {
        console.error(e);
        alert("Connection Error! Please check your Firebase Rules.");
    }
}

function handleLogout() { if(confirm("Logout?")) { localStorage.removeItem('sp_user'); localStorage.removeItem('sp_state_backup'); window.location.reload(); } }
...
async function saveState() {
    ...
    const status = document.getElementById('sync-status');
    if (status) status.innerHTML = '<i class="fas fa-sync fa-spin"></i> Securing...';
    
    try {
        state.password = currentUser.pass;
        await db.ref(`shops/${currentUser.id}`).set(state);
        if (status) status.innerHTML = '<i class="fas fa-check-circle"></i> Cloud Secured';
    } catch (e) { 
        console.error("Save Error:", e);
        if(status) status.innerHTML = '<i class="fas fa-wifi-slash"></i> Sync Failed'; 
    }
}
...
function startLiveSync() {
    ...
    syncRef = db.ref(`shops/${currentUser.id}`);
    syncRef.on('value', (snapshot) => {
        const data = snapshot.val();
        if (data && data.password === currentUser.pass) {
        ...
        alert("Firebase Sync Error! Make sure your 'Realtime Database Rules' are set to true in the console.");
    });
}
...
function openBulkReminderModal() {
    ...
    bulkQueue = (state.borrowers || []).filter(b => {
        const paid = (b.payments || []).reduce((a, c) => a + Number(c.amount), 0);
        return (Number(b.amount) - paid) > 700;
    }).map(b => {
        const paid = (b.payments || []).reduce((a, c) => a + Number(c.amount), 0);
    ...
    if (bulkQueue.length === 0) {
        listDiv.innerHTML = '<p style="text-align:center; padding:1rem; color:var(--text-muted);">No borrowers found with balance > ₹700.</p>';
        if(sendBtn) sendBtn.disabled = true;
    } else {
        listDiv.innerHTML = bulkQueue.map((b, i) => `
            <div style="display:flex; justify-content:space-between; padding:0.8rem; border-bottom:1px solid var(--border); background:rgba(255,255,255,0.02);">
                <span>${b.name}</span>
        ...
        if(sendBtn) {
            ...
            sendBtn.innerHTML = `Start Sending (${bulkQueue.length} People)`;
        }
    }
    showModal('bulkReminderModal');
}

function sendNextBulkReminder() {
    if (bulkQueue.length === 0) {
        alert("All reminders sent or list is empty!");
        closeModal();
        return;
    }

    const b = bulkQueue.shift(); // Get the first person
    ...
    // Update button text for next one
    const sendBtn = document.getElementById('bulkSendBtn');
    if (sendBtn) {
        if (bulkQueue.length > 0) {
            sendBtn.innerHTML = `Send Next (${bulkQueue.length} Remaining)`;
        } else {
            sendBtn.innerHTML = `Finish`;
        }
    }
    ...
    const listDiv = document.getElementById('bulkReminderList');
    if (listDiv) {
        listDiv.innerHTML = bulkQueue.map((bq) => `
            <div style="display:flex; justify-content:space-between; padding:0.8rem; border-bottom:1px solid var(--border); background:rgba(255,255,255,0.02);">
                <span>${bq.name}</span>
                <span class="gold" style="font-weight:bold;">₹${bq.balance}</span>
            </div>
        `).join('');
    }
