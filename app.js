    ...
    list.innerHTML = (state.borrowers || []).filter(b => b.name.toLowerCase().includes(search) || (b.phone && b.phone.includes(search))).map((b, i) => {
        ...
        return `<tr><td>${b.name}</td><td>${b.phone||''}</td><td>₹${b.amount}</td><td>₹${paid}</td><td class="gold">₹${bal}</td>
            <td><div style="display:flex; gap:0.2rem; flex-wrap:wrap;">
                ...
                <button class="btn btn-primary" onclick="remindBorrowerSMS(${i})" style="padding:0.2rem 0.4rem; font-size:0.65rem; background:var(--accent); color:black;"><i class="fas fa-comment-alt"></i></button>
...
function renderBulkModalState() {
    ...
    if (!listDiv || !progressText || !actionBtn) return;
    ...
    if (bulkQueue.length === 0) {
        ...
        actionBtn.innerHTML = '<i class="fas fa-check-circle"></i> Nothing to Send';
        return;
    }
    ...
    if (currentQueueIndex < bulkQueue.length) {
        ...
        if (!isAutoSending) {
            progressText.innerHTML = `Sending <strong>${currentQueueIndex + 1}</strong> of <strong>${bulkQueue.length}</strong> reminders`;
            actionBtn.disabled = false;
            actionBtn.style.background = '#25D366';
            actionBtn.style.color = 'black';
            actionBtn.innerHTML = `<i class="fas fa-play"></i> Start Auto Custom SMS (5s Delay)`;
        }
    } else {
        ...
        actionBtn.disabled = true;
        actionBtn.style.background = 'var(--border)';
        actionBtn.style.color = 'var(--text-muted)';
        actionBtn.innerHTML = '<i class="fas fa-check-circle"></i> Queue Finished';
    }
}
...
function updateCountdownUI() {
    const progressText = document.getElementById('bulkProgressText');
    const actionBtn = document.getElementById('bulkActionBtn');
    if (!progressText || !actionBtn) return;

    if (currentQueueIndex < bulkQueue.length) {
        const nextCustomer = bulkQueue[currentQueueIndex];
        progressText.innerHTML = `<span style="color:#25D366; font-weight:bold;"><i class="fas fa-spinner fa-spin"></i> Next reminder to ${nextCustomer.name} in ${countdownSeconds}s...</span>`;
        actionBtn.innerHTML = `<i class="fas fa-pause"></i> Pause Auto Sending`;
...
function sendGroupSMS() {
    ...
    const phones = list.map(b => b.phone.trim()).filter(p => p.length > 0);
    if (phones.length === 0) {
