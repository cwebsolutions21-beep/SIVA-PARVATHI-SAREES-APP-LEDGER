...
function loadUltraMsgKeys() {
    const instance = localStorage.getItem('ultramsg_instance') || '';
    const token = localStorage.getItem('ultramsg_token') || '';
    
    const instanceInput = document.getElementById('ultraMsgInstance');
    const tokenInput = document.getElementById('ultraMsgToken');
    
    if (instanceInput) instanceInput.value = instance;
    if (tokenInput) tokenInput.value = token;
}

function saveUltraMsgKeys() {
    const instanceInput = document.getElementById('ultraMsgInstance');
    const tokenInput = document.getElementById('ultraMsgToken');
    
    if (instanceInput) localStorage.setItem('ultramsg_instance', instanceInput.value.trim());
    if (tokenInput) localStorage.setItem('ultramsg_token', tokenInput.value.trim());
}

function toggleUltraMsgTokenVisibility() {
    const input = document.getElementById('ultraMsgToken');
    const icon = document.getElementById('ultraMsgTokenEyeIcon');
    if (!input || !icon) return;
...
async function sendWhatsAppAPIBulk() {
    const instance = (localStorage.getItem('ultramsg_instance') || '').trim();
    const token = (localStorage.getItem('ultramsg_token') || '').trim();

    if (!instance || !token) {
        alert("Please enter your UltraMsg Instance ID and Token on the Dashboard settings card first! (You can get them in 2 minutes for free at ultramsg.com)");
        return;
    }
    ...
    if (!confirm(`Are you sure you want to send 100% automated background WhatsApp reminders to all ${bulkQueue.length} customers?`)) {
        return;
    }
    ...
    for (let i = 0; i < bulkQueue.length; i++) {
        const b = bulkQueue[i];
        if (progressText) {
            progressText.innerHTML = `<span style="color:#00e676; font-weight:bold;"><i class="fas fa-paper-plane fa-spin"></i> Sending background reminder to ${b.name} (${i + 1}/${bulkQueue.length})...</span>`;
        }

        const msg = `నమస్కారం ${b.name}, శివ పార్వతి సారీస్ షాపు నుండి గుర్తు చేయునది. మీ బాకీ మొత్తం ₹${b.balance} ఇంకా చెల్లించాల్సి ఉంది. ధన్యవాదాలు!`;
        const formattedPhone = formatWhatsAppPhone(b.phone);

        try {
            // Call the UltraMsg API
            ...
            if (response.ok) {
                ...
                console.error("Failed to send message to", b.name);
            }
        } catch (err) {
            console.error("Error calling UltraMsg API:", err);
        }

        // Add a small 1-second delay between background messages to prevent WhatsApp spam flags
        await new Promise(resolve => setTimeout(resolve, 1000));
    }

    if (statusCard) statusCard.style.display = 'none';
    alert(`Successfully sent ${successCount} out of ${bulkQueue.length} WhatsApp reminders automatically in the background!`);
    resetBulkQueue();
}
