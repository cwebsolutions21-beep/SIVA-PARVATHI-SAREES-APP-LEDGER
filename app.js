...
function loadFast2SMSKey() {
    const key = localStorage.getItem('fast2sms_api_key') || '';
...
function remindBorrower(i) {
    ...
    window.open(`https://api.whatsapp.com/send?phone=${formatWhatsAppPhone(b.phone)}&text=${encodeURIComponent(msg)}`, '_blank');
}
...
function sendNextSingleTabReminderDirectly() {
    ...
    if (activeReminderMode === 'whatsapp') {
        // Open/reuse in WhatsApp named window (100% Free!)
        window.open(`https://api.whatsapp.com/send?phone=${formatWhatsAppPhone(b.phone)}&text=${encodeURIComponent(msg)}`, 'whatsapp_sender_tab');
    } else {
