...
function renderBulkModalState() {
    ...
    if (currentQueueIndex < bulkQueue.length) {
        ...
        if (!isAutoSending) {
            ...
            actionBtn.innerHTML = `<i class="fas fa-play"></i> Start Auto Sending (5s Delay)`;
        }
    } else {
...
function sendNextSingleTabReminderDirectly() {
    ...
    // Open in a named window ('whatsapp_sender_tab') so it always reuses the same browser tab
